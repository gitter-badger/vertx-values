...
class FindAndUpdateActors {
    ...
    protected Supplier<Function<UpdateMessage, Future<JsObj>>> spawnFindOneAndUpdate() {
        Function<UpdateMessage, JsObj> fn = m ->
                collection.get()
                          .findOneAndUpdate(objVal2Bson.apply(m.filter),
                                            objVal2Bson.apply(m.update)
    ...
    protected Supplier<Function<UpdateMessage, Future<JsObj>>> spawnFindOneAndUpdate(final FindOneAndUpdateOptions options,
                                                                                     final DeploymentOptions deploymentOptions) {
        Function<UpdateMessage, JsObj> fn = m ->
                collection.get()
                          .findOneAndUpdate(objVal2Bson.apply(m.filter),
