...
class FindAndReplaceActors {
    ...
    protected Supplier<Function<UpdateMessage, Future<JsObj>>> spawnFindOneAndReplace() {
        Function<UpdateMessage, JsObj> fn = m ->
                collection.get()
                          .findOneAndReplace(objVal2Bson.apply(m.filter),
                                             m.update
    ...
    protected Supplier<Function<UpdateMessage, Future<JsObj>>> spawnFindOneAndReplace(final FindOneAndReplaceOptions options,
                                                                                      final DeploymentOptions deploymentOptions) {
        Function<UpdateMessage, JsObj> fn = m ->
                collection.get()
                          .findOneAndReplace(objVal2Bson.apply(m.filter),
